...
class fbank_addUser:
    ...
    def bank_addUser(self,account,username,password,country,province,street,door,money):
        ...
        if not user:
            sql3 = "insert into ban(account, username, password, country, province, street, door, money, bankname) values (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
            param3 = [account, username, password, country, province, street, door, money,self.__bank_name]
            update(sql3,param3)
            return 1
        else:
            return 2
...
# 开户的输入数据
class zbank_addUser(fbank_addUser):
    def addUser(self):
        ...
        if status == 3:
            ...
            info = '''
                ...
                ---------------------------------------
            '''
            print(info % (data[0][1], data[0][0], data[0][2], data[0][3], data[0][4], data[0][5],data[0][6], data[0][7],
            data[0][8],data[0][9]))
...
#逻辑定为父类
class Fbank_quq:
    def bank_quq(self,account1, password1,jin):
...
class Fban_cx:
    def bank_cx(self,account2,password2):
...
class Zbank_cx(Fban_cx):
    def cx(self):
        account2=int(input("请输入您的账号"))
        password2=int(input("请输入您的密码"))
        status=super().bank_cx(account2,password2)
        if status==1:
            print("该用户不存在")
        elif status==3:
            print("您输入的密码错误")
